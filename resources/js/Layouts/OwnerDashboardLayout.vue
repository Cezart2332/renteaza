                        ...
                        >
                            ...
                            >
                                ...
                                <div class="tw-px-4 tw-pt-3 tw-pb-2">
                                    <ToggleWithLabel
                                        v-model:role="dashboardRole"
                                        class="tw-w-full tw-flex tw-justify-center tw-ml-2"
                                    />
                                </div>
        ...
        >
            ...
            >
                <div>
                    ...
                    <ToggleWithLabel
                        v-model:role="dashboardRole"
                        class="tw-flex tw-gap-3 tw-justify-center"
                    />
                    ...
                    <nav class="tw-flex tw-flex-1 tw-flex-col tw-mt-4">
                        ...
                        >
                            <li>
                                ...
                                <!-- CLIENT -->
                                <ul
                                    v-else-if="dashboardRole === 'Client'"
                                    role="list"
                                    class="tw--mx-2 tw-space-y-1"
                                >
                                    <li
...
// prioritar ce vine din server; fallback pe prop-ul primit sau 'Client'
const initialRole = computed(
    () =>
        page.props.selectedRole ??
        (typeof props?.selectedRole === "string"
            ? props.selectedRole
            : "Client")
);
const dashboardRole = ref(initialRole.value);

// dacă serverul schimbă selectedRole (ex. după POST), ținem UI-ul în sync
watch(
    () => page.props.selectedRole,
    (val) => {
        if (val && val !== dashboardRole.value) dashboardRole.value = val;
    }
);

// apelat automat de ToggleWithLabel via v-model:role
watch(dashboardRole, (role) => {
    // persistă rolul în sesiune pe backend
    router.post(
        route("user.dashboard.setRole"),
        { role },
        {
            preserveState: true,
            preserveScroll: true,
...
const ownerRoutes = [
    ...
    // { name: 'Venituri', href: '#', icon: CalendarIcon, current: false },
    {
        name: "Profil",
