        ...
        <div class="tw-space-y-6">
            ...
            <div class="tw-flex tw-items-end tw-justify-between">
                ...
                <div class="tw-flex tw-justify-start tw-space-x-6">
                    ...
                    <template v-else-if="vehicle.status === 'active'">
                        <button
                            type="button"
                            @click="updateStatus('inactive')"
                            class="tw-bg-red-500 hover:tw-bg-red-600 tw-text-white tw-px-8 tw-py-3 tw-rounded-lg tw-text-sm tw-font-semibold tw-shadow"
                    ...
                    <template v-else-if="vehicle.status === 'inactive'">
                        <button
                            @click="updateStatus('active')"
                            class="tw-bg-green-500 hover:tw-bg-green-600 tw-text-white tw-px-8 tw-py-3 tw-rounded-lg tw-text-sm tw-font-semibold tw-shadow"
                        >
...
function updateStatus(status) {
    router.put(
        route("admin.users.vehicle-status.update", props.vehicle),
        {
            status: status,
        },
        {
...
function submitFeedback() {
    ...
    props.allowedTypes.forEach((type) => {
        if (!documentData.value[type].admin_comment && comments.value[type]) {
            payload.comments[type] = comments.value[type];
        }
    });

    // snapshot pentru posibil rollback
    const prev = {};
    Object.keys(payload.comments).forEach((type) => {
        prev[type] = {
            admin_comment: documentData.value[type]?.admin_comment ?? null,
            verified_at: documentData.value[type]?.verified_at ?? null,
        };
    });

    router.post(
        route("admin.users.vehicle-documents.comment", {
        ...
        payload,
        {
            ...
            onSuccess: () => {
                ...
                Object.entries(payload.comments).forEach(([type, comment]) => {
                    if (!documentData.value[type])
                        documentData.value[type] = {};
                    documentData.value[type].admin_comment = comment;
                    documentData.value[type].verified_at = now;
                    comments.value[type] = "";
                });
            },

            onError: () => {
                Object.entries(prev).forEach(([type, data]) => {
                    if (!documentData.value[type])
                        documentData.value[type] = {};
                    documentData.value[type].admin_comment = data.admin_comment;
                    documentData.value[type].verified_at = data.verified_at;
                });
            },
        }
    );
